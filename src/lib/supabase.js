...
export async function listCustomDomains(){const u=getSession()?.user?.id;if(!u)return [];return request(`/rest/v1/custom_domains?user_id=eq.${encodeURIComponent(u)}&select=*&order=created_at.desc`)}
export async function createCustomDomain(domain,invitationId=null){const u=getSession()?.user?.id;if(!u)throw Error('Please sign in first.');const d=await request('/rest/v1/custom_domains',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({user_id:u,domain:domain.trim().toLowerCase(),invitation_id:invitationId||null})});return d?.[0]||d}
...
export async function listInvitations(){const u=getSession()?.user?.id;if(!u)return [];return request(`/rest/v1/invitations?owner_id=eq.${encodeURIComponent(u)}&select=*,invitation_gallery(*),rsvps(*)&order=created_at.desc`)}
...
export async function createInvitation(p){try{const d=await request('/rest/v1/invitations',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(p)});return d?.[0]||d}catch(e){if(String(e.message)==='PRODUCT_NOT_PURCHASED')throw Error('هذا القالب غير مملوك لحسابك. اشترِ القالب من المتجر أولاً.');throw e}}
export async function updateInvitation(id,p){try{const d=await request(`/rest/v1/invitations?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({...p,updated_at:new Date().toISOString()})});return d?.[0]||d}catch(e){if(String(e.message)==='PRODUCT_NOT_PURCHASED')throw Error('هذا القالب غير مملوك لحسابك. اشترِ القالب من المتجر أولاً.');throw e}}
export async function deleteInvitation(id){await request(`/rest/v1/invitations?id=eq.${encodeURIComponent(id)}`,{method:'DELETE'})}
export async function submitRsvp(id,r){const d=await request('/rest/v1/rsvps',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({invitation_id:id,guest_name:r.name.trim(),attending:r.attending==='yes',guests_count:Math.min(20,Math.max(1,Number(r.guests||1))),note:r.note?.trim()||null})});return d?.[0]||d}
export async function listInvitationEvents(id){if(!id)return [];return request(`/rest/v1/invitation_events?invitation_id=eq.${encodeURIComponent(id)}&select=*&order=sort_order.asc,event_time.asc`)}
export async function replaceInvitationEvents(id,events=[]){await request(`/rest/v1/invitation_events?invitation_id=eq.${encodeURIComponent(id)}`,{method:'DELETE'});if(!events.length)return [];return request('/rest/v1/invitation_events',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(events.map((e,i)=>({invitation_id:id,event_time:e.event_time?.trim()||'',title:e.title?.trim()||'',description:e.description?.trim()||null,sort_order:i})))})}
function safe(n='image'){return n.toLowerCase().replace(/[^a-z0-9._-]+/g,'-').replace(/^-+|-+$/g,'')||'image'}
export async function uploadInvitationMedia(file){if(!file||!file.type?.startsWith('image/'))throw Error('Only image files are supported.');if(file.size>8*1024*1024)throw Error('Please choose an image smaller than 8 MB.');const s=getSession(),u=s?.user?.id;if(!u)throw Error('Please sign in before uploading images.');const {url,key}=config(),f=`${crypto.randomUUID()}-${safe(file.name)}`,x=await fetch(`${url}/storage/v1/object/invitation-media/${encodeURIComponent(u)}/${encodeURIComponent(f)}`,{method:'POST',headers:{apikey:key,Authorization:`Bearer ${s.access_token||key}`,'Content-Type':file.type,'x-upsert':'false'},body:file});if(!x.ok)throw Error('Image upload failed.');return {path:`${u}/${f}`,url:`${url}/storage/v1/object/public/invitation-media/${encodeURIComponent(u)}/${encodeURIComponent(f)}`}}
export async function replaceGallery(id,urls=[]){await request(`/rest/v1/invitation_gallery?invitation_id=eq.${encodeURIComponent(id)}`,{method:'DELETE'});if(!urls.length)return [];return request('/rest/v1/invitation_gallery',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(urls.map((url,i)=>({invitation_id:id,url,sort_order:i})))})}
